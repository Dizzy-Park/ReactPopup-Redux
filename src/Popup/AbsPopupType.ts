...
import type { IPopupState } from "./store/absPopupVo";
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function addPopup<T = any>(
  key: string,
  popup: React.LazyExoticComponent<React.ComponentType<T>>
) {
  popups[key] = popup;
}

export function addPopupString(key: string, link: string) {
  popups[key] = lazy(() => import(link));
}
