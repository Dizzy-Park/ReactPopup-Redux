...
import { AbsPopupType, type IPopupStore } from "../AbsPopupType";
import {
  rdxPopupClose,
  rdxPopupOpen,
...
import { useSelectorEq } from "../common";
...
export function useClosePopup(type: AbsPopupType | string) {
  const { callBack, returnData } = useSelectorEq((state: IPopupStore) => ({
...
>(type: AbsPopupType | string) {
  const { popupDo } = useSelectorEq((state: IPopupStore) => ({
...
export function useAbsPopupTitle(type: AbsPopupType | string) {
  const { title, isClose } = useSelectorEq((state: IPopupStore) => ({
...
export function useAbsPopupButton(type: AbsPopupType | string) {
  const { buttonOption, Button } = useSelectorEq((state: IPopupStore) => ({
 ...
 */
function usePopup() {
  const { isPopup, popupDo } = useSelectorEq((state: IPopupStore) => ({
...
export function usePopupOpen() {
  const dispatch = useDispatch();
  return <T>(popupDo: IPopupDo<T>) => {
    dispatch(rdxPopupOpen(popupDo));
  };
}
...
export function useSetPopupData() {
  const dispatch = useDispatch();
  return <T>(type: string, data: T) => {
    dispatch(
      rdxSetPopupData({
        type: type,
        value: {
...
export function useRemoveOnePopupData() {
  const dispatch = useDispatch();
